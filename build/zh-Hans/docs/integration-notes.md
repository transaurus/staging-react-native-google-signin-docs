...
## Additional scopes[​](#additional-scopes "Direct link to Additional scopes")

The default requested scopes are `email` and `profile`.

If you want to manage other data from your application (for example, access user agenda or upload a file to drive) you need to request additional permissions. This can be accomplished by calling [`requestAuthorization`](/zh-Hans/docs/one-tap.md#requestauthorization) or by adding the necessary scopes when calling [`configure`](/zh-Hans/docs/one-tap.md#configure).
