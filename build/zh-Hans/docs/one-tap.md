...
This is Google's recommended way to implement Google Sign In. This API is available on Android, iOS, macOS and web (with a little extra work [described below](#web-support)). It is a replacement for the [Original Google sign in](/zh-Hans/docs/original.md). The module APIs are named `GoogleOneTapSignIn` for historical reasons.
...
* On the web, it covers both the [One-tap](https://developers.google.com/identity/gsi/web/guides/offerings#one_tap) flow and the [Google Sign-In button](https://developers.google.com/identity/gsi/web/guides/offerings#sign_in_with_google_button). [Learn more](/zh-Hans/docs/web-support.md).
...
signature: (`params`: [`OneTapConfigureParams`](/zh-Hans/docs/api.md#onetapconfigureparams)) => `void`
...
signature: (`params`?: [`OneTapSignInParams`](/zh-Hans/docs/api.md#onetapsigninparams)) => `Promise`<[`OneTapResponse`](/zh-Hans/docs/api.md#onetapresponse)>
...
* [`isErrorWithCode`](/zh-Hans/docs/errors.md#iserrorwithcodevalue) for processing errors
* [`isSuccessResponse`](/zh-Hans/docs/api.md#issuccessresponse) for checking if a response represents a successful operation. Same as checking `response.type === 'success'`.
* [`isNoSavedCredentialFoundResponse`](/zh-Hans/docs/api.md#isnosavedcredentialfoundresponse) for checking if a response represents no saved credentials case. Same as checking `response.type === 'noSavedCredentialFound'`.
* [`isCancelledResponse`](/zh-Hans/docs/api.md#iscancelledresponse) for checking if a response represents user cancellation case. Same as checking `response.type === 'cancelled'`.

***

### `createAccount`[​](#createaccount "Direct link to createaccount")

signature: (`params`?: [`OneTapCreateAccountParams`](/zh-Hans/docs/api.md#onetapcreateaccountparams)) => `Promise`<[`OneTapResponse`](/zh-Hans/docs/api.md#onetapresponse)>
...
Use `createAccount` if `signIn` resolved with [`NoSavedCredentialFound` result](/zh-Hans/docs/api.md#nosavedcredentialfound), as indicated in the code snippet above.
...
signature: (`params`?: [`OneTapExplicitSignInParams`](/zh-Hans/docs/api.md#onetapexplicitsigninparams)) => `Promise`<[`OneTapExplicitSignInResponse`](/zh-Hans/docs/api.md#onetapexplicitsigninresponse)>

| Platform | Behavior                                                                                                                                                                                                                                                                                                                       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Android  | Presents the sign in dialog explicitly. This is useful if both `signIn` and `createAccount` resolve with [`NoSavedCredentialFound`](/zh-Hans/docs/api.md#nosavedcredentialfound) object - which happens (in the unlikely case) when no Google account is present on the device. This prompts the user to add a Google account. |
| Apple    | Starts an interactive sign-in flow. Same as `createAccount`.                                                                                                                                                                                                                                                                   |
| Web      | Presents a one-tap prompt. Same as `createAccount`.                                                                                                                                                                                                                                                                            |

Preferably, call this method only as a reaction to when user taps a [sign in button](/zh-Hans/docs/buttons/google-logo-button.md).
...
signature: (`showErrorResolutionDialog`?: `boolean`): `Promise`<[`PlayServicesInfo`](/zh-Hans/docs/api.md#playservicesinfo)>
...
![prompt install](/zh-Hans/assets/images/prompt-install-b928a962a8d2c0a416628bffd0c95448.png) ![prompt enable](/zh-Hans/assets/images/prompt-enable-812fb1d8cb868511c8b8baccb13306e0.png)

* Apple: Play Services are an Android-only concept and are not needed on Apple. Hence, the method always resolves.
* Web: resolves when the Google Client Library [is loaded](/zh-Hans/docs/setting-up/web.md), rejects otherwise.
...
signature: (`params`: [`RequestAuthorizationParams`](/zh-Hans/docs/api.md#requestauthorizationparams)) => `Promise`<[`AuthorizationResponse`](/zh-Hans/docs/api.md#authorizationresponse)>
...
| Apple    | Calls [`addScopes`](/zh-Hans/docs/original.md#addscopes). The resulting `accessToken` has access to the requested scopes. Use this if you want to read more user metadata than just the basic info.                                        |
...
[Learn more](/zh-Hans/docs/web-support.md).
